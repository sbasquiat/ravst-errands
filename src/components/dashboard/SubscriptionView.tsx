...
interface SubscriptionViewProps {
  currentPlan: string;
  monthlyErrands: number;
  monthlySpent: number;
}

export default function SubscriptionView({ currentPlan, monthlyErrands, monthlySpent }: SubscriptionViewProps) {
  const [selectedPlan, setSelectedPlan] = useState(currentPlan);
  const [upgradeLoading, setUpgradeLoading] = useState(false);

  const handleUpgrade = async () => {
    if (selectedPlan === currentPlan || selectedPlan === "pay_as_you_go") return;
      ...
      >
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="mt-1 text-lg font-bold text-[var(--color-charcoal)]">{planNames[currentPlan] ?? currentPlan}</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-[var(--color-text-light)]">This month</p>
            <p className="text-lg font-bold text-[var(--color-charcoal)]">{monthlyErrands} errands · €{monthlySpent.toFixed(2)}</p>
          </div>
        </div>
        {currentPlan === "pay_as_you_go" && monthlyErrands >= 4 && (
          <div className="mt-4 rounded-xl bg-amber-50 px-4 py-3">
            <p className="text-sm text-amber-700">
              <span className="font-semibold">Tip:</span> You&apos;d save money on the Starter Bundle with your current usage!
            </p>
          </div>
        )}
      </motion.div>
      ...
      {selectedPlan !== currentPlan && (
        ...
        >
          {selectedPlan === "pay_as_you_go" ? (
            <button
              onClick={handleDowngradeToPayg}
              className="flex items-center gap-2 rounded-xl border border-[var(--color-border)] bg-white px-6 py-3 text-sm font-semibold text-[var(--color-text)] transition-all hover:bg-[var(--color-cream)] cursor-pointer"
            >
